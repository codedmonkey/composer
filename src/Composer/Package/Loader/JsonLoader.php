...
class JsonLoader implements LoaderInterface
{
    ...
    private function createObject(JsonPackage $config, string $class): CompletePackage
    {
        ...
        if ($config->versionNormalized !== null) {
     ...
     */
    private function configureObject(PackageInterface $package, JsonPackage $config): BasePackage
    {
        if (!$package instanceof CompletePackage) {
            throw new \LogicException('JsonLoader expects instances of the Composer\Package\CompletePackage class to function correctly');
        }

        $package->setType($config->type !== null ? strtolower($config->type) : 'library');
        ...
        $package->setIsDefaultBranch($config->defaultBranch);
        ...
        $package->setDevAutoload($config->autoloadDev);
        $package->setIncludePaths($config->includePath);
        $package->setPhpExt($config->phpExt);
        ...
        $package->setNotificationUrl($config->notificationUrl);;
        ...
        if ($this->loadOptions) {
            $package->setTransportOptions($config->transportOptions);
        }
     ...
     */
    public function getBranchAlias(JsonPackage $config): ?string
    {
